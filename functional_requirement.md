...
*Use case 뒤에 #가 붙어있으면 이미 elaboration 완료된 Use case, + {이름} 이 붙어있으면 특정인에게 할당된 Usecase를 의미함.*

1. 회원가입과 로그인
2. 유저가 프로젝트를 개설#
3. 프로젝트의 유저가 이슈를 조회회
4. 프로젝트의 유저가 이슈를 검색#
5. 프로젝트의 유저가 전체 발생한 이슈의 통계를 조회
6. 프로젝트의 유저가 이슈의 상세 정보를 확인#

  \- 이슈의 상태와 코멘트를 확인 가능

6. 프로젝트의 유저가 이슈에 코멘트를 달고, 자신이 단 코멘트를 관리함#
7. Reporter가 이슈를 등록#
...
+ Basic Flow
      ...
      1. PA가 인터페이스를 통해 자신에게 할당된 이슈를 조회한다.

      2. PA가 할당된 이슈에 대한 정보를 확인한다.

      3. PA가 할당된 이슈를 fixed 상태로 변경시킨다.
